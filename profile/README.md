...
| Step | Section | What to look for |
|---:|---|---|
| 1 | [Case File](#case-file) | ClueRoom이 어떤 게임인지 |
| 2 | [Product Surface](#product-surface) | 사용자가 보는 화면과 플레이 진입점 |
| 3 | [Investigation Loop](#investigation-loop) | 한 판의 사건 조사 흐름 |
| 4 | [What We Built](#what-we-built) | 제품 기능과 구현 범위 |
| 5 | [System Shape](#system-shape) | Web/App에서 백엔드, 데이터, 관측까지 이어지는 구조 |
| 6 | [AI Safety & LLMOps](#ai-safety--llmops) | AI가 정답을 모르게 만드는 내부 경계 |
| 7 | [Proof Snapshot](#proof-snapshot) | 테스트, QA, 성능, LLMOps 검증 숫자 |
| 8 | [Operations & Delivery Evidence](#operations--delivery-evidence) | 운영 자동화와 산출물 추적 근거 |
| 9 | [Stack](#stack) | 내부 구현에 사용한 기술 |
| 10 | [Repositories](#repositories) | 코드 저장소 경계 |
| 11 | [Team Responsibilities](#team-responsibilities) | 팀 역할과 담당 범위 |
| 12 | [Roadmap](#roadmap) | 남은 보강 방향 |
| 13 | [Quick Links](#quick-links) | 외부 링크와 저장소 바로가기 |
...
<p align="center">
  <img src="assets/web-scenario-detail-cropped.png" alt="ClueRoom web scenario detail" width="49%">
  <img src="assets/web-case-hub.png" alt="ClueRoom web investigation hub" width="49%">
</p>

**React/Vite Web production surface.** 공개 README에는 스포일러 위험이 낮은 시나리오 상세와 수사 허브 화면만 사용합니다.
...
|---:|---:|---:|---:|---:|---:|
| **345 PASS** | **32 PASS** + OAuth/session recovery | **23 tests** + core smoke PASS | **25/25 · 35/35** evidence reachability | P95 **241ms -> 19ms** | **95 / 0 / 0** QA-window success/failure/fallback + Redis AI quota |

> Public-safe 보고 기준에 따라 정답, 점수, session/token, raw prompt/answer는 공개 README에 포함하지 않습니다.
> Web core smoke는 주요 플레이 경로와 bookmark persistence 기준입니다. review success path와 refresh-cookie retry는 배포 후 gate로 따로 확인합니다.
> LLMOps 95건 표본은 웹 QA가 섞인 provider window라 organic traffic 평균으로 해석하지 않습니다.

### Snapshot 읽는 법

- Backend: test, performance, scale-out, LLMOps 근거를 백엔드 README와 public-safe 보고서에 연결합니다.
- Web: core smoke와 bookmark persistence는 PR #7 운영 smoke 기준 PASS이고, review success path와 refresh-cookie retry는 별도 gate입니다.
- AI/LLMOps: `95 / 0 / 0`은 QA-window provider 표본이며 일반 사용자 traffic 평균이 아닙니다.
- Ops: scale-out 수치는 운영 기본 구조가 아니라 수동 LB PoC 검증 결과입니다.
...
<table>
  <tr>
    <td width="50%">
      ...
      <p><strong>운영 스냅샷 자동화</strong><br>active slot, data/server health, backup age, suspicious request pattern을 Slack에서 바로 확인할 수 있게 정리했습니다.</p>
    </td>
    <td width="50%">
      <img src="assets/github-readme-polish-pr-trace.png" alt="ClueRoom GitHub PR and merge notification trace" width="100%">
      <p><strong>GitHub/Jira 알림 흐름</strong><br>README polish, PR merge, task transition이 팀 Slack에 남아 산출물과 작업 이력을 추적할 수 있습니다.</p>
    </td>
  </tr>
</table>
...
|---|---|---|
| Web Client | [`clueroom-web-fe`](https://github.com/Final-Project-sixteam-company/clueroom-web-fe) | React/Vite 웹 배포 화면과 OAuth 기반 플레이 진입점 |
| Android Client | [`project-fe`](https://github.com/Final-Project-sixteam-company/project-fe) | Flutter Android 앱, 모바일 플레이 UX, Android E2E 근거 |
| Backend Core | [`start-up-project`](https://github.com/Final-Project-sixteam-company/start-up-project) | Spring Boot API, AI/gameplay domain, auth, infra, LLMOps docs |
...
```text
1. Web production smoke 마감: review success path, refresh-cookie retry
...
| Android App | [Repository](https://github.com/Final-Project-sixteam-company/project-fe) |
